...
public class Main extends InputAdapter implements ApplicationListener {
    ...
    private void input() {
        ...
        if(Gdx.input.isKeyJustPressed(Input.Keys.UP)) {
            ...
            int guraNextYInTiles = (int) (guraYInTiles + 1);
        ...
        if(Gdx.input.isKeyJustPressed(Input.Keys.LEFT)) {
            isColliding = false;
            int guraNextXInTiles = (int) (guraXInTiles - 1);
        ...
        float maxCameraX = (float) ((TiledMapTileLayer) tiledMap.getLayers().get(0)).getWidth() - (float) (camera.viewportWidth / 2);
        float maxCameraY = (float) ((TiledMapTileLayer) tiledMap.getLayers().get(0)).getHeight() - (float) (camera.viewportHeight / 2);
        ...
        if (camera.position.y > (float) (camera.viewportHeight / 2)) {
            if (guraSprite.getY() <= minY + tileHeight / 2) {
                camera.position.y -= tileHeight;
                guraSprite.translateY(tileHeight);
            }
        }
        if (camera.position.x > (float) (camera.viewportWidth / 2)) {
